...
--
--  PODE RODAR QUANTAS VEZES QUISER. Todo comando aqui é idempotente:
--  as tabelas e índices usam `if not exists`, e cada policy é derrubada
--  antes de ser recriada. Então, sempre que este arquivo mudar, cole ele
--  inteiro de novo — não é preciso caçar o trecho novo. Nenhum dado é
--  tocado: não há `drop table`, `delete` nem `alter column` em lugar nenhum.
--
--  A ordem importa e é esta: tabelas primeiro (cada uma depois daquelas a
--  que ela se refere), depois índices, RLS, policies e por fim o gatilho.
-- ============================================================
...
--  `enable row level security` não reclama se já estiver ligado.
-- ============================================================
...
-- ------------------------------------------------------------
--  POLICIES
--  O Postgres não tem `create policy if not exists`, então cada uma é
--  derrubada antes de ser recriada. É o que permite colar este arquivo
--  inteiro de novo sem o erro 42710 ("policy already exists").
-- ------------------------------------------------------------
...
-- MESES
drop policy if exists "mes_select" on public.meses;
drop policy if exists "mes_insert" on public.meses;
drop policy if exists "mes_update" on public.meses;
drop policy if exists "mes_delete" on public.meses;
...
-- CONTAS FIXAS
drop policy if exists "fix_select" on public.fixos;
drop policy if exists "fix_insert" on public.fixos;
drop policy if exists "fix_update" on public.fixos;
drop policy if exists "fix_delete" on public.fixos;
...
drop policy if exists "pul_select" on public.fixos_pulados;
drop policy if exists "pul_insert" on public.fixos_pulados;
drop policy if exists "pul_delete" on public.fixos_pulados;
...
-- ENTRADAS EXTRAS
drop policy if exists "ent_select" on public.entradas;
drop policy if exists "ent_insert" on public.entradas;
drop policy if exists "ent_update" on public.entradas;
drop policy if exists "ent_delete" on public.entradas;
...
-- ANÁLISES
drop policy if exists "ana_select" on public.analises;
drop policy if exists "ana_insert" on public.analises;
drop policy if exists "ana_update" on public.analises;
drop policy if exists "ana_delete" on public.analises;
...
-- GASTOS
drop policy if exists "gasto_select" on public.gastos;
drop policy if exists "gasto_insert" on public.gastos;
drop policy if exists "gasto_update" on public.gastos;
drop policy if exists "gasto_delete" on public.gastos;
...
-- LIXEIRA
-- Não há policy de update: um registro na lixeira é uma fotografia do que
-- foi apagado. Ou ele volta inteiro, ou é descartado — nunca é editado.
